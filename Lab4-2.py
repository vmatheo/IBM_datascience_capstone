...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        filtered_df = spacex_df.groupby('Launch Site')['class'].agg(['count'])
        filtered_df = filtered_df.reset_index()
        fig = px.pie(data=filtered_df, values='count', 
        names='Launch Site', 
        title='Successes for each Launch Site')
        return fig
    else:
...
def get_scatter_plot(entered_site):
    if entered_site == 'ALL':
        fig = px.scatter(data=spacex_df, x='Payload Mass (kg)', 
        y='Class',
